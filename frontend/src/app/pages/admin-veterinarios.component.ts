...
@Component({
  ...
  styleUrl: './admin-veterinarios.component.css',
})
export class AdminVeterinariosComponent {
  ...
  cargarClientes(): void {
    this.usuarioService.getVeterinarios().subscribe({
      next: (data) => {
        this.clientes = data;
      },
      error: () => {
  ...
  actualizarCliente(): void {
    ...
    this.usuarioService
      ...
      .subscribe({
        next: () => {
          this.showAlert('Veterinario actualizado correctamente', 'success');
          this.cargarClientes();
        ...
        error: (err) => {
          const errorMsg =
            err.error?.message || 'Error al actualizar veterinario';
